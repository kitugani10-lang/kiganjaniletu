...
import { Shield, Trash2, CheckCircle, XCircle, UserPlus, UserMinus } from 'lucide-react';
...
const Moderation = () => {
  ...
  useEffect(() => {
    if (hasAccess) { fetchReports(); fetchRoles(); }
  }, [hasAccess, fetchReports, fetchRoles]);
  ...
  const handleAddModerator = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('user_roles').insert({
        user_id: profile.id, role: 'moderator',
      });
      if (error) {
        ...
        <Tabs defaultValue="pending">
                ...
                <TableBody>
                  {resolvedReports.map(report => (
                    <TableRow key={report.id}>
                      <TableCell>
                        <Badge variant="outline">{report.post_id ? 'Post' : 'Comment'}</Badge>
                      </TableCell>
          ...
          {isAdmin && (
            <TabsContent value="team">
              <Card className="mb-4">
                <CardHeader>
                  <CardTitle className="text-lg">Add Moderator</CardTitle>
                </CardHeader>
                <CardContent>
